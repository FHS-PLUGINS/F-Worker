...
class TRSWork extends PluginBase implements Listener {
		...
		if($event->getBlock ()->getId () == 19 && $event->getBlock ()->getDamage () == 0) {
				...
				switch ($prize) {
					case 1 :
						...
						$player->sendPopup ( "§a> §6빵 §f1개를 지급 받았습니다." );
						$level->addSound ( new ClickSound ( $player ) );
						break;
					case 2 :
                        $inventory->addItem ( Item::get ( 297, 0, 2 ) ); // 쿠키
                        $player->sendPopup ( "§a> §6빵 §f2개를 지급 받았습니다." );
						$level->addSound ( new ClickSound ( $player ) );
						break;
					case 3 :
						$player->sendPopup ( "§c> §6빵§f이 지급 되지 않았습니다." );
						break;
					case 4 :
						$player->sendPopup ( "§c> §6빵§f이 지급 되지 않았습니다." );
						break;
